...
describe("summarizeEpicReport", () => {
  const mockedCreateChatCompletion =
  ...
  test("should summarize an epic report", async () => {
    const epicReportString = fs.readFileSync(
      "tests/unit/data/epicReport.json",
      "utf-8",
    );
    ...
    expect(summary).toEqual({
      choices: [
        {
          finish_reason: "stop",
          index: 0,
          message: { content: "i am a human", role: "assistant" },
        },
      ],
      created: 1699708896,
      id: "chatcmpl-8JiDYs8wa9sDuv2jW4qXOX5573d6Q",
      ...
      object: "chat.completion",
      system_fingerprint: "fp_a24b4d720c",
      usage: { completion_tokens: 47, prompt_tokens: 369, total_tokens: 416 },
    });
  });
});
