...
def analyze_epic(epic_key, last_checked_date):
    ...
    changelogs = fetch_issue_query_changelogs(f'key = {epic_key}', max_results=1)
    result['epic_changelog'] = changelogs[0]['changelog'] if changelogs else None
    ...
    for child in child_issues['issues']:
        ...
        if changelog:
          ...
          current_status = child.get('fields').get('status').get('name')
          for log in filtered_changelog:
            for item in log['items']:
                if item['field'] == 'status' and item['toString'] == 'In Progress' and current_status == 'In Progress':
                    in_progress_date = datetime.strptime(log['created'], '%Y-%m-%dT%H:%M:%S.%f%z')
                    if (datetime.now() - in_progress_date).days > 10:
                        long_running_issues.append(child['id'])
